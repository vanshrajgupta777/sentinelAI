...
export default function TaskDetailPage() {
  ...
  const { backendReachable, backendMode } = useApp();
  ...
  const onDownload = () => {
    if (!task.result?.approvalNote && !task.result) return;
    // Prefer the real DOCX artifact generated by the backend.
    const artifactId = (task as any).artifactId || (task as any).artifact_id;
    if (backendReachable && artifactId) {
      window.location.href = api.artifactUrl(artifactId);
      return;
    }
    // Fallback: local text download.
