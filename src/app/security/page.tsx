...
export default function SecurityPage() {
  const { connections, audit } = useApp();
      ...
      <Card>
        <div className="flex flex-col items-center gap-3 border-b border-slate-200 pb-6 text-center">
          ...
          <p className="max-w-2xl text-sm text-slate-600">
            Local processing active. SentinelAI Workbench is operating within the secure enclave
            with no external AI communication.
          </p>
        </div>
        <div className="grid grid-cols-2 gap-3 pt-6 sm:grid-cols-4">
          <BigStat label="External Connections" value="0" accent="emerald" icon={<Network className="h-4 w-4" />} />
          <BigStat label="External AI APIs" value="0" accent="emerald" icon={<Shield className="h-4 w-4" />} />
          <BigStat label="External Data Transfers" value="0" accent="emerald" icon={<Lock className="h-4 w-4" />} />
          <BigStat label="Local Services" value="4" accent="cyan" icon={<Server className="h-4 w-4" />} />
        </div>
      </Card>
